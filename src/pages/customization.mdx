...
2. When the opinion greatly tampers with a developer's experience
...
Instead of having your source code in the root directory, you can put it in a child directory. Add the directory name as the resolved `paths` for the `~*` prefix in the `tsconfig.json` file, and the Typescript compiler will know how to resolve your code. The example configuration below reads source code from the `src` sub-directory:
