...
- `~rulesets/test.json` used in the [manifest overide inside `package.json`](/customization#overriding-the-manifest) resolves to `/rulesets/test.json`
- `data-base64:~assets/image.png` resolves to `/assets/image.png`
- `url:~src/code.js` resolves to `/src/code.js.`

If the tilde is by itself without a scheme and is being used to import a source code module from another source code module (`ts` or `tsx` files), it has two behaviors depending on whether you are using the default setup or the [the `src` directory setup](/customization#using-src-directory-for-source-code):
