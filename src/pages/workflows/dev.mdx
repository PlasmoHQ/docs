...
For a list of officially supported targets, [click here](./faq#what-are-the-officially-supported-browser-targets).

## Without source maps

By default, Plasmo generates source maps for your extension during development. If for some reason you need to disable this, you can use the `--no-source-maps` flag:

```sh
plasmo dev --no-source-maps
```

This is useful when you have dependencies that does not play well with source maps.
