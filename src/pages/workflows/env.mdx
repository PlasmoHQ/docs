...
If there is a `CRX_PUBLIC_KEY` variable in `.env.development` but not in `.env.production` or `.env.local`, then it will only be available in `plasmo dev` and not `plasmo build`.
