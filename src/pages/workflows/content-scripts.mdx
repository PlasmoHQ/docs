...
In the example above, we use the `data-text` scheme to load our stylesheet as plain-text into the cssText variable, which we then assign to our style element's textContent. Be cautious with the import path when using this scheme if you use the `src` directory, as noted [here](/workflows/assets#/workflows/faq#tilde-import-resolution).
