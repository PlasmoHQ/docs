...
By using a function instead of a static value, the initial value will be persisted into storage memory.

Let's say we have a `popup.tsx` that initialize the state to "42":

```tsx
const [hailingFrequency, setHailingFrequency] = useStorage("hailing", async () => "42")
...
{hailingFrequency} // "42"
```
