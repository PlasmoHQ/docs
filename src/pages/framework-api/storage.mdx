...
#### Preventing data from being mirroed into localStorage
...
This prevents data from being leaked into localStorage when using with content-scripts. It's recommended to use this feature if you are storing JWT access token to invoke your server-side API.
...
Given a `popup.tsx` that set a static initial value:
