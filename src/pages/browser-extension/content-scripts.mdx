...
Content scripts run in the context of web pages under an "isolated world." This allows multiple content scripts from various extensions to co-exist without conflicting with each other's execution.
...
See [with-content-script](https://github.com/PlasmoHQ/examples/tree/main/with-content-script) for another example.
...
Content scripts created with Plasmo are statically declared scripts by default. To provide a custom content script configuration, export a config object from your content script:
...
The main usecase for this config is to tell the browser how to load this specific content-script, i.e which site should it run on, should it run on `about:blank` page, and which page should be included or excluded. To learn more about the config and what each property does, [click here](https://developer.chrome.com/docs/extensions/mv3/content_scripts/#static-declarative).
