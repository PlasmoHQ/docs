...
Plasmo has built-in support for most common import path such as relative import, `node_module` package import, `ESM exports` import. It also provides project-relative paths via tilde and absolute imports. To further customize your import path, checkout the [Alias Imports Section](/customization/alias).
...
To import files using paths relative to the current file, use the `./` prefix. For example, to import a file from the same directory as the existing file, use `./file-name`. To import a file from a subdirectory, use `./subdirectory/file-name`. To import a file from a parent directory, use `../file-name`.
...
> NOTE: `raw-env` is mostly useful for text-based assets.
...
> Using the `url` with image assets will optimize them for size and browser loading.
