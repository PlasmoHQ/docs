...
**WARNING:** This doc is heavily outdated (note sections that were marked "OUTDATED"). Please inspect the types exported by Plasmo for more information, and go through the CSUI mounting logic in the framework to study how it works. We have been swarmed by work lately and haven't had the time to update this doc. If you have some time, please consider contributing by opening a PR 😄
...
Since the `getOverlayAnchor` function is async, you also have the power to control _when_ Plasmo mounts your component. For example, you can wait for a specific element to appear on the page before mounting your component.
...
- Extension needs to use an [iframe](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/iframe), instead
