...
Starting from Plasmo `v0.65.0`, Plasmo extension may specify a `world` within the config:

You must inject code into the main world if you'd like to modify the `window` object from your content script.

To manually inject a script, you can also use the `chrome.scripting.executeScript` API. First, add the scripting permission in your [package.json's 'manifest.permissions' array](/framework/customization/manifest):
