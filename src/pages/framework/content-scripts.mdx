...
Because content scripts run within the context of a web page, they are subject to the same-origin policy. To bypass CORS restrictions, you can use the [Plasmo Messaging API](/framework/messaging#message-flow) to proxy the request via your background service worker.
...
> NOTE: Please see this [note about `~` import resolution](/framework/workflows/faq#tilde-import-resolution)
