...
Any `import` inside the injected `func` will not be resolved at the moment. This is because the entire function must be scoped entirely within the `executeScript` call.

</Callout>

See [with-scripting](https://github.com/PlasmoHQ/examples/tree/main/with-scripting) for an example.
