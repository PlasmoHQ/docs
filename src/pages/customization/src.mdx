...
Creating a Plasmo project with a `src` directory setup is as simple as:
...
Please ensure that **ALL source files** including Plasmo's entry files such as `popup.tsx`, `options.tsx`, `background.ts`, `contents/*` etc. are in the `src` directory. Otherwise, Plasmo **WILL NOT** know where to find the entry files, which will result in an empty extension!
